...
export default function Locations() {
    ...
    const canadaLocations = restaurants.filter((restaurant) => {
        ...
        const aIsRestobar = (a.slug === 'restobar' || String(a.name || '').toLowerCase().includes('byward') || String(a.name || '').toLowerCase().includes('resto bar'));
        const bIsRestobar = (b.slug === 'restobar' || String(b.name || '').toLowerCase().includes('byward') || String(b.name || '').toLowerCase().includes('resto bar'));
        if (aIsRestobar && !bIsRestobar) return -1;
        if (!aIsRestobar && bIsRestobar) return 1;
        return 0;
    });
