...
export default function Reservations() {
  ...
  useEffect(() => {
    api.getRestaurants()
      .then((data) => {
        const restobarOnly = (data || []).filter((r) => (
          r?.slug === 'restobar'
          || String(r?.name || '').toLowerCase().includes('restobar')
          || String(r?.name || '').toLowerCase().includes('resto bar')
        ));
        ...
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="grid sm:grid-cols-3 gap-6 mt-12">
            {[
              ...
              { icon: Phone, title: 'Need Help?', desc: 'Email infomasakaliottawa@gmail.com for assistance.' },
            ].map((item, i) => {
