...
// Parse menu.json into flat arrays that the component expects
function parseMenuJson(data) {
  const rawCategories = Object.values(data.categories || {});
  const rawItemsArray = Array.isArray(data.items) ? data.items : Object.values(data.items || {});
  const rawItems = {};
  for (const item of rawItemsArray) {
    if (item && item.id) rawItems[item.id] = item;
  }
  ...
  // Build flat items array, respecting each category's item order
  const items = [];
  const seenItemIds = new Set();

  for (const cat of categories) {
    const rawCat = data.categories[cat.id];
    if (!rawCat?.items) continue;

    for (const itemId of rawCat.items) {
      const raw = rawItems[itemId];
      if (!raw || !raw.available) continue;
      // An item can belong to multiple categories; add it under each
      const compositeKey = `${cat.id}::${itemId}`;
      if (seenItemIds.has(compositeKey)) continue;
      seenItemIds.add(compositeKey);
      ...
      items.push({
        ...
        price: Number.isFinite(raw.price) ? Number((raw.price / 100).toFixed(2)) : 0,
        image_url: Array.isArray(raw.images) && raw.images.length > 0 ? raw.images[0].source : null,
...
export default function Menu() {
  ...
  useEffect(() => {
    try {
      const parsed = parseMenuJson(menuData);
